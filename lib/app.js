...
if (!config.SERVER_ADDRESS || !config.SESSION_SECRET || !config.EMAIL_ACCOUNT || !config.EMAIL_PASSWORD || !config.MONGODB) {
...
app.use(function(req, res, next) {
  ...
  if (req.get('x-forwarded-proto') === 'https' || req.secure === true || host === 'localhost:8080') {
    return next();
  }
